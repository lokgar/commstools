...
## Visual Pipeline Tour

Below is a walkthrough of a complete single-carrier transmission pipeline—from signal generation, through impairment modeling, matched filtering, and symbol demodulation—showcasing CommsTools' rich visualization capabilities.

### 1. Signal Generation & Spectral Analysis

We generate a pulse-shaped 16-QAM signal and pass it through a noisy channel modeled with AWGN at $E_s/N_0 = 16\text{ dB}$.

```python
from commstools import Signal
from commstools.impairments import apply_awgn

# 1. Generate pulse-shaped 16-QAM signal (oversampled at sps=4)
sig = Signal.qam(
    order=16,
    num_symbols=5000,
    sps=4,
...
# 3. Plot PSD of transmitter vs. noisy receiver spectrum
...
### 2. Matched Filtering & Eye Diagram

At the receiver, we apply a Matched Filter matched to our RRC pulse shape. We visualize the resulting signal quality using a high-density 2D eye diagram.

```python
# 1. Apply Matched Filter (derives filter taps automatically from signal metadata)
noisy_matched = noisy.copy().matched_filter()

# 2. Plot Eye Diagram (Vectorized 2D density histogram)
...
![Eye Diagram](examples/images/eye_diagram.png)

---

### 3. Slicing, Constellation & EVM Metrics
