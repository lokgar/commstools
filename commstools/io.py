...
def save_npz(
    ...
    *,
    ...
    Notes
    -----
    When the signal was generated by ``to_signal``,
    the originating frame is serialised into the archive
    (``__frame_metadata__`` + ``frame_payload_symbols`` etc.).  On load,
    ``load_npz`` reconstructs the frame and re-attaches it so that
    ``correct_timing`` and frame-aware equalizer workflows work
