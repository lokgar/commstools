...
def main():
    ...
    coarse_est, frac_est = estimate_timing(sig_rx.samples, reference=tx_samples, sps=SPS)
    
    # Apply inverse timing correction
    sig_rx.samples = correct_timing(sig_rx.samples, coarse_offset=coarse_est, fractional_offset=frac_est, mode="circular")
    ...
    sig_rx.samples = correct_static_frequency_offset(sig_rx.samples, sampling_rate=SAMPLING_RATE, offset=fo_est)
    ...
    axes[0, 1].plot(time_axis, to_device(samples_noisy.real[:150], "cpu"), label="I", alpha=0.8)
    axes[0, 1].plot(time_axis, to_device(samples_noisy.imag[:150], "cpu"), label="Q", alpha=0.8)
