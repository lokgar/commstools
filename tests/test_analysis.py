...
def _wiener_phase(linewidth_hz, n, seed=0):
    """Discrete Wiener phase walk at the symbol rate (NumPy, float64)."""
    rng = np.random.default_rng(seed)
    incr = rng.normal(0.0, np.sqrt(2.0 * np.pi * linewidth_hz * T), n)
...
def test_linewidth_increment_slope_awgn_free(xp):
    ...
    assert res["linewidth_hz"] == pytest.approx(dnu, rel=0.10)
...
def test_linewidth_beta_separation_floor(xp):
    ...
    assert out["linewidth_floor_hz"] == pytest.approx(dnu, rel=0.10)
...
def test_frequency_drift_metrics_sinusoid(xp):
    ...
    assert m["std_hz"] == pytest.approx(amp / np.sqrt(2.0), rel=0.05)
    assert m["pp_hz"] == pytest.approx(2.0 * amp, rel=0.10)
...
def test_separate_drift_phase_noise_splits(xp):
    ...
    phi = slow + fast
    drift, pn = analysis.separate_drift_phase_noise(xp.asarray(phi), R, cutoff_hz=1e6)
...
def test_characterize_carrier_phase_report(xp):
    ...
    for key in (
        ...
        assert key in rep
    assert rep["linewidth_increment"]["linewidth_hz"] == pytest.approx(dnu, rel=0.15)
